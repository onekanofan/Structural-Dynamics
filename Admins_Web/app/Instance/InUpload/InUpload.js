...
angular.module('InUpload', ['helper'])
    .config(function ($stateProvider) {
        $stateProvider.state('Instance.Upload', {
            url : '/Upload',
            views : {
                'sidenv' : {
                    templateUrl : 'app/Instance/Instance.html',
                    controller : 'InUploadSidenvCtrl'
                },
                'content' : {
                    templateUrl : 'app/Instance/InUpload/InUpload.html',
                    controller : 'InUploadCtrl'
                }
            }
        });
    })

    .controller('InUploadSidenvCtrl', function ($scope) {
        $scope.selectedPage = 'Upload';
        document.getElementById($scope.selectedPage).className = 'list_item sel_item';
    })
    .controller('InUploadCtrl', function ($scope, FileUploader, AppConstants) {
        localStorage.setItem('Instance','Upload');

        var uploader = $scope.uploader = new FileUploader({
            url : AppConstants.URL_BASE + '/upload/instance',
            method : "POST",
            alias : "zip_file",
            headers : {
